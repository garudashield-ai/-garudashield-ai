...
export default async function ApkReportPage({
  params,
}: {
  ...
  if (!report) {
    return (
      <div className="min-h-screen bg-black text-white pt-32 px-6 flex flex-col items-center">
        ...
        <Link href="/apk" className="text-emerald-500 mt-4 underline">
          Kembali
        </Link>
      </div>
    );
  ...
  if (report.status === "processing") {
    const dummyFile = new File([""], report.target);
    return (
      <ApkScanningView id={id} file={dummyFile} isPrivate={report.isPrivate} />
    );
  }
  const getStatusIcon = () => {
